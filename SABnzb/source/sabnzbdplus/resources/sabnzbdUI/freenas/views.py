...
def start(request, plugin_id):
    (sabnzbd_key, sabnzbd_secret) = utils.get_sabnzbd_oauth_creds()

    url = utils.get_rpc_url(request)

    trans = OAuthTransport(url, key=sabnzbd_key,
        secret=sabnzbd_secret)
    ...
    try:
        form = forms.SabnzbdForm(sabnzbd.__dict__, instance=sabnzbd, jail_path=jail_path)
        form.is_valid()
        form.save()
    except ValueError:
        return HttpResponse(simplejson.dumps({
            'error': True,
            'message': 'Sabnzbd data did not validate, please configure it first.',
            }), content_type='application/json')

    cmd = "%s onestart" % utils.sabnzbd_control
...
def stop(request, plugin_id):
    ...
    cmd = "%s onestop" % utils.sabnzbd_control
...
def edit(request, plugin_id):
    ...
    if form.is_valid():
        ...
        cmd = "%s restart" % utils.sabnzbd_control
        pipe = Popen(cmd, stdin=PIPE, stdout=PIPE, stderr=PIPE, shell=True, close_fds=True)

        return JsonResponse(request, error=True, message="Sabnzbd settings successfully saved.")
...
def treemenu(request, plugin_id):
    ...
    url = utils.get_rpc_url(request)
    trans = OAuthTransport(url, key=sabnzbd_key, secret=sabnzbd_secret)
    server = jsonrpclib.Server(url, transport=trans)
    ...
    plugin = {
        ...
        'type': 'pluginsfcgi',
        'url': reverse('sabnzbd_edit', kwargs={'plugin_id': plugin_id}),
        'kwargs': {'plugin_name': 'sabnzbd', 'plugin_id': plugin_id},
    }
...
def treemenu_icon(request, plugin_id):
    with open(utils.sabnzbd_icon, 'rb') as f:
        icon = f.read()
