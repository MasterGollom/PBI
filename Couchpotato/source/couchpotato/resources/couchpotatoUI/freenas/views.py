...
from couchpotatoUI.freenas import forms, models, utils
...
def start(request, plugin_id):
    (couchpotato_key, couchpotato_secret) = utils.get_couchpotato_oauth_creds()

    url = utils.get_rpc_url(request)

    trans = OAuthTransport(url, key=couchpotato_key,
        secret=couchpotato_secret)
    ...
    try:
        couchpotato = models.Couchpotato.objects.order_by('-id')[0]
        couchpotato.enable = True
        couchpotato.save()
    except IndexError:
        couchpotato = models.Couchpotato.objects.create(enable=True)

    try:
        form = forms.CouchpotatoForm(couchpotato.__dict__, instance=couchpotato, jail_path=jail_path)
        form.is_valid()
        form.save()
    except ValueError:
        return HttpResponse(simplejson.dumps({
            'error': True,
            'message': 'Couchpotato data did not validate, please configure it first.',
            }), content_type='application/json')

    cmd = "%s onestart" % utils.couchpotato_control
...
def stop(request, plugin_id):
    ...
    try:
        couchpotato = models.Couchpotato.objects.order_by('-id')[0]
        couchpotato.enable = False
        couchpotato.save()
    except IndexError:
        couchpotato = models.Couchpotato.objects.create(enable=False)
    ...
    cmd = "%s onestop" % utils.couchpotato_control
...
def edit(request, plugin_id):
    ...
    """
    Get the Couchpotato object
    If it does not exist create a new entry
    """
    try:
        couchpotato = models.Couchpotato.objects.order_by('-id')[0]
    except IndexError:
        couchpotato = models.Couchpotato.objects.create()
    ...
    if request.method == "GET":
        form = forms.CouchpotatoForm(instance=couchpotato,
            jail_path=jail_path)
    ...
    form = forms.CouchpotatoForm(request.POST,
        instance=couchpotato,
        jail_path=jail_path)
    if form.is_valid():
        form.save()

        cmd = "%s restart" % utils.couchpotato_control
        pipe = Popen(cmd, stdin=PIPE, stdout=PIPE, stderr=PIPE, shell=True, close_fds=True)

        return JsonResponse(request, error=True, message="Couchpotato settings successfully saved.")
...
def treemenu(request, plugin_id):
    ...
    url = utils.get_rpc_url(request)
    trans = OAuthTransport(url, key=couchpotato_key, secret=couchpotato_secret)
    server = jsonrpclib.Server(url, transport=trans)
    ...
    plugin = {
        ...
        'type': 'pluginsfcgi',
        'url': reverse('couchpotato_edit', kwargs={'plugin_id': plugin_id}),
        'kwargs': {'plugin_name': 'couchpotato', 'plugin_id': plugin_id},
    }
...
def status(request, plugin_id):
    ...
    proc = Popen(["/usr/bin/pgrep", "-u", "media"], stdout=PIPE, stderr=PIPE)
...
def treemenu_icon(request, plugin_id):
    with open(utils.couchpotato_icon, 'rb') as f:
        icon = f.read()
