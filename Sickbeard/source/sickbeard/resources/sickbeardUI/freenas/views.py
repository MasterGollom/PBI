...
def start(request, plugin_id):
    ...
    try:
        sickbeard = models.Sickbeard.objects.order_by('-id')[0]
        sickbeard.enable = True
        sickbeard.save()
    except IndexError:
    ...
    try:
        ...
        return HttpResponse(simplejson.dumps({
            'error': True,
            'message': 'Sickbeard data did not validate, please configure it first.',
...
def stop(request, plugin_id):
    ...
    try:
        sickbeard = models.Sickbeard.objects.order_by('-id')[0]
        sickbeard.enable = False
        sickbeard.save()
    except IndexError:
        sickbeard = models.Sickbeard.objects.create(enable=False)
    ...
    cmd = "%s onestop" % utils.sickbeard_control
...
def edit(request, plugin_id):
    ...
    try:
        ...
        sickbeard = models.Sickbeard.objects.create()
...
def treemenu(request, plugin_id):
    ...
    plugin = {
        ...
        'type': 'pluginsfcgi',
        'url': reverse('sickbeard_edit', kwargs={'plugin_id': plugin_id}),
        'kwargs': {'plugin_name': 'sickbeard', 'plugin_id': plugin_id},
    }
...
def status(request, plugin_id):
    ...
    proc = Popen(["/usr/bin/pgrep", "-u", "_sickbeard"], stdout=PIPE, stderr=PIPE)
...
def treemenu_icon(request, plugin_id):
    with open(utils.sickbeard_icon, 'rb') as f:
        icon = f.read()
